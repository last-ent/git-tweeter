...
def getCommitMessage(path: String = "/Users/Ent/Documents/pynik/ScalaBase/git-tweeter") = {
  ...
  val headPath = Source.fromFile(gitPath + "HEAD").mkString.split(" ")(1).trim
  val commitHash = Source.fromFile(gitPath + headPath).mkString.trim
  val commitFile = new File(gitPath + "objects/" + commitHash.slice(0,2)+"/"+commitHash.drop(2))
